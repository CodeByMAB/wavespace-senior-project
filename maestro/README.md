...
These flows automate the five critical paths referenced by QA ticket `8aac7435-def5-4837-bc7f-b1094e676254`. Flows 3–4 use conditional steps so a cold install (onboarding only) still passes file parsing; run against a device or emulator with a dev client where `appId` matches `app.json` (`com.anonymous.wavespace`).
